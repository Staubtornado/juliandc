...
class Song:
    ...
    def create_embed(self):
        ...
        embed = (discord.Embed(title=f'🎶 {self.source.title}', description=f'[Song]({self.source.url}) | [{self.source.uploader}]({self.source.uploader_url}) | {self.source.duration} | {self.requester.mention}', color=0xef2811)
                .add_field(name='Aufrufe', value=millify(int(self.source.views), precision=2, prefixes=prefixes), inline=True)
                .add_field(name='Likes / Dislikes', value=f'{millify(int(self.source.likes), precision=2, prefixes=prefixes)} / {millify(int(self.source.dislikes), precision=2, prefixes=prefixes)}', inline=True)
                .add_field(name='Hochgeladen', value=f'<t:{str(datetime.datetime(int(year), int(month), int(day), 0, 0).timestamp())[:-2]}:R>', inline=True)
...
class Music(commands.Cog):
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        if len(ctx.voice_state.songs) > 120 or psutil.virtual_memory().percent > 75:
            return await ctx.send(embed = discord.Embed(title = 'Hohe Auslastung...', description = f'In der Warteschlange befinden sich {len(ctx.voice_state.songs)}. Es können maximal 120 Songs in die Warteschlange aufgenommen werden.', colour = 0xFF0000) .set_footer(text = f'{round(psutil.virtual_memory().used / (1024.0 ** 2))}MB / {round(psutil.virtual_memory().total / (1024.0 ** 2))}MB'))
        ...
        if "https://open.spotify.com/playlist/" in search or "spotify:playlist:" in search:
            async with ctx.typing():
                try:
                    trackcount = 0
                    process = await ctx.send(embed = discord.Embed(description = '<a:loading:820216894703009842> Playlist wird untersucht...', 
                        colour = 0x1db954) 
                        .set_author(name = 'Spotify®', icon_url='https://media.discordapp.net/attachments/797127597132742668/866246117875646474/Spotify_Icon_RGB_Green.png?width=676&height=676'))
                    ...
                    for track in tracks:
                        ...
                        try:
                            ...
                            await ctx.send('Ein Fehler ist bei der Verarbeitung aufgetreten: {}'.format(str(e)))
                        else:
                            song = Song(source)
                            await ctx.voice_state.songs.put(song)

                    await process.edit(embed = discord.Embed(description = f':white_check_mark: `{trackcount}` Songs wurden hinzugefügt...', 
                        colour = 0x1db954) 
                        .set_author(name = 'Spotify®', icon_url='https://media.discordapp.net/attachments/797127597132742668/866246117875646474/Spotify_Icon_RGB_Green.png?width=676&height=676'))
            ...
            async with ctx.typing():
                process = await ctx.send(embed = discord.Embed(description = '<a:loading:820216894703009842> Album wird untersucht...', 
                    colour = 0x1db954) 
                    .set_author(name = 'Spotify®', icon_url='https://media.discordapp.net/attachments/797127597132742668/866246117875646474/Spotify_Icon_RGB_Green.png?width=676&height=676'))
                try:
                    ...
                    await process.edit(embed = discord.Embed(description = f':white_check_mark: `{len(tracks)}` Songs wurden hinzugefügt...', 
                        colour = 0x1db954) 
                        .set_author(name = 'Spotify®', icon_url='https://media.discordapp.net/attachments/797127597132742668/866246117875646474/Spotify_Icon_RGB_Green.png?width=676&height=676'))
            ...
            async with ctx.typing():
                process = await ctx.send(embed = discord.Embed(description = '<a:loading:820216894703009842> Track wird untersucht...', 
                    colour = 0x1db954) 
                    .set_author(name = 'Spotify®', icon_url='https://media.discordapp.net/attachments/797127597132742668/866246117875646474/Spotify_Icon_RGB_Green.png?width=676&height=676'))
                try:
                    ...
                    song = Song(source)
                    await ctx.voice_state.songs.put(song)
                    await process.edit(embed = discord.Embed(description = f':white_check_mark: `{track}` wurde hinzugefügt...', 
                        colour = 0x1db954) 
                        .set_author(name = 'Spotify®', icon_url='https://media.discordapp.net/attachments/797127597132742668/866246117875646474/Spotify_Icon_RGB_Green.png?width=676&height=676'))
            ...
            async with ctx.typing():
                process = await ctx.send(embed = discord.Embed(description = '<a:loading:820216894703009842> Künstler wird untersucht...', 
                    colour = 0x1db954) 
                    .set_author(name = 'Spotify®', icon_url='https://media.discordapp.net/attachments/797127597132742668/866246117875646474/Spotify_Icon_RGB_Green.png?width=676&height=676'))
                try:
                    ...
                    for result in results['tracks'][:10]:
                        ...
                        await process.edit(embed = discord.Embed(description = f':white_check_mark: `{len(results["tracks"][:10])}` Songs von `{Spotify.getArtist(self, search)}` wurden hinzugefügt...', 
                            colour = 0x1db954) 
                            .set_author(name = 'Spotify®', icon_url='https://media.discordapp.net/attachments/797127597132742668/866246117875646474/Spotify_Icon_RGB_Green.png?width=676&height=676'))
            ...
            async with ctx.typing():
                ...
                try:
                    ...
                    for video in playlist.items:
                        ...
                        try:
                            source = await YTDLSource.create_source(ctx, str(video.snippet.resourceId)[42:][:11], loop=self.bot.loop)
                        except YTDLError as e:
                            await ctx.send('Ein Fehler ist bei der Verarbeitung aufgetreten: {}'.format(str(e)))
                        else:
                            song = Song(source)
                    ...
                    await process.edit(embed = discord.Embed(description = f':white_check_mark: `{len(playlist.items)}` Songs wurden hinzugefügt...', 
                        colour = 0x1db954))
